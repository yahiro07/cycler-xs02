...
export function clampValueZeroOne(val: number) {
...
export function mapUnaryBipolar(val: number) {
  return val * 2 - 1;
}
...
//   if (v1 === v2) return v1;
//   const steps = Math.abs(v2 - v1) + 1;
//   const idx = Math.min(Math.floor(val * steps), steps - 1);
//   const dir = v2 > v1 ? 1 : -1;
//   return v1 + dir * idx;
// }
// export function mapUnaryToValues(pr: number, values: number[]): number {
//   const index = Math.min((pr * values.length) >>> 0, values.length - 1);
//   return values[index];
// }

//Math.roundによる0.5境界での分割
export function mapUnaryToInt(val: number, v1: number, v2: number) {
...
export function invPower2Weak(x: number, a: number) {
  const y1 = x;
  const y2 = 1 - (1 - x) * (1 - x);
...
export function power4(x: number) {
  return x * x * x * x;
}

export function invPower4(x: number) {
  return 1 - (1 - x) * (1 - x) * (1 - x) * (1 - x);
}

export function roundFloat(value: number, frac: number) {
  return parseFloat(value.toFixed(frac));
}
